/* ===================
   FOR THREE BOT - STORE V3 FINAL
   =================== */
const TU_NUMERO_WSP = '51936994155'; // <-- TU NUMERO SIN +
...
const CUENTAS = {
  yape: { num:'936994155', titular:'For Three' },
  bank: { cci:'00219100234567890123', cuenta:'191-23456789-0-12', titular:'For Three' },
  prex: { num:'948572136', titular:'For Three' },
  paypal: { link:'https://paypal.me/ForThree', usd: 0.28 }
};
...
function renderSubnav(){ // <- CREA LOS 3 BOTONES
  $('#subnav').innerHTML = `<button data-tab="todos" class="active">TODOS</button><button data-tab="bot">BOT</button><button data-tab="serv">SERVICIOS</button>`;
  $$('.subnav button').forEach(b=>b.onclick=()=>{ $$('.subnav button').forEach(x=>x.classList.remove('active')); b.classList.add('active'); state.tab=b.dataset.tab; render(); });
}
...
function show(pantalla){ $$('.screen').forEach(s=>s.classList.remove('active')); $(`#${pantalla}`).classList.add('active'); }

function render(){ // <- USA #productos NO #grid
  const grid = $('#productos');
  let lista = PRODUCTOS.filter(p=> state.tab==='todos' || p.cat===state.tab );
...
function verProducto(id){
  ...
  $('#pImg').src = p.img; $('#pMarca').textContent = p.marca; $('#pNom').textContent = p.nom;
...
function abrirPago(){
  ...
  $('#serviceCard').innerHTML = `<img src="${p.img}"><div><b>${p.nom}</b><div class="precio">S/ ${p.precio.toFixed(2)}</div></div>`;
  $('#yapeNum').textContent = CUENTAS.yape.num; $('#yapeName').textContent = CUENTAS.yape.titular;
  $('#bcpCci').textContent = CUENTAS.bank.cci; $('#bcpCuenta').textContent = CUENTAS.bank.cuenta; $('#bcpNombre').textContent = CUENTAS.bank.titular;
  $('#prexNum').textContent = CUENTAS.prex.num; $('#prexName').textContent = CUENTAS.prex.titular;
  $('#paypalLink').href = CUENTAS.paypal.link;
  state.descuentoAplicado = { code: null, percent: 0 }; $('#couponCode').value = ''; $('#couponApplied').style.display = 'none'; $('#couponInput').style.display = 'flex';
  selectPayment('yape'); $('#pagoModal').classList.add('active'); actualizarMontos();
}
...
function applyCoupon(){
  ...
  if(!CUPONES[code]) return toast('Cupón inválido', true);
  state.descuentoAplicado = { code, percent: CUPONES[code] };
  $('#couponText').textContent = `${code} -${CUPONES[code]}%`; $('#couponInput').style.display = 'none'; $('#couponApplied').style.display = 'flex';
  actualizarMontos(); toast(`Cupón ${code} aplicado`);
}
...
function actualizarMontos(){
  const precio = getPrecioFinal(); const usd = (precio * CUENTAS.paypal.usd).toFixed(2);
  $('#montoYape').textContent = `S/ ${precio.toFixed(2)}`; $('#montoTarjeta').textContent = `S/ ${precio.toFixed(2)}`;
  $('#montoPrex').textContent = `S/ ${precio.toFixed(2)}`; $('#montoPaypal').textContent = `$${usd} USD`;
}

function selectPayment(tab, e){
  state.metodoPago = tab; // <- GUARDA SI ES YAPE/BANK/PREX/PAYPAL
  $$('.payment-tab').forEach(b=>b.classList.remove('active')); if(e) e.target.classList.add('active');
  $$('.payment-content').forEach(c=>c.classList.remove('active')); $(`#${tab}Content`).classList.add('active');
  actualizarMontos();
}
...
function guardarVenta(){
  ...
  const venta = { id: 'F3-' + Date.now(), producto: state.productoSeleccionado.nom, precio_soles: precioFinal, metodo: state.metodoPago, cupon: state.descuentoAplicado.code || 'Ninguno', fecha: new Date().toLocaleString('es-PE', { timeZone: 'America/Lima' }), estado: 'PENDIENTE' };
  ...
  cerrarPago(); toast('Pedido enviado al Bot ✅');
}
$('#wspBtn').onclick = guardarVenta; // <- CONECTA EL BOTON
...
window.verProducto = verProducto; window.comprar = comprar; window.abrirPago = abrirPago; window.cerrarPago = cerrarPago;
window.applyCoupon = applyCoupon; window.removeCoupon = removeCoupon; window.selectPayment = selectPayment; window.copyText = copyText; window.show = show;
